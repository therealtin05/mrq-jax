...
def add(
    ...
    env_idx = jnp.arange(add_batch_size)[:, None]
...
def sample(
    ...
    max_time = jnp.where(state.is_full, max_length_time_axis, state.current_index) # (add_batch_size,)
    # When full, the oldest valid data is current_index otherwise it is zero.
    head = jnp.where(state.is_full, state.current_index, 0) # (add_batch_size,)

    # Given no wrap around, the last valid starting index is:
    max_start = max_time - sequence_length # (add_batch_size,)
    ...
    num_valid_items = jnp.where(max_start >= 0, (max_start // period) + 1, 0) # (add_batch_size,)
    ...
    sampled_item_idx = jax.random.randint(
        subkey_items, (batch_size,), 0, num_valid_items[sampled_batch_indices]
    ) # (batch_size,)
    # Compute the logical start time index: ls = (sampled_item_idx * period).
    logical_start = sampled_item_idx * period # (batch_size,)
    # Map logical time to physical index in the buffer given there is wrap around.
    physical_start = (
        head[sampled_batch_indices] + logical_start
    ) % max_length_time_axis # (batch_size,)
